...
```{r fgsea}
fgsea <- pfgsea(statsList = stats_list, gmtFiles = gmt_files)
saveData(fgsea, dir = params$data_dir)
```

```{r export_csv}
# Naming system: fgsea_c1_mutant_vs_control.csv
for (gmt in seq_len(length(fgsea))) {
    contrasts <- fgsea[[gmt]]
    for (contrast in seq_len(length(contrasts))) {
        x <- fgsea[[gmt]][[contrast]]
        name <- paste(
            "fgsea",
            names(fgsea)[[gmt]],
